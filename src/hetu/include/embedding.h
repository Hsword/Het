...
public:
  ...
  Line(cache_key_t key, size_t len, bool init_data = true) : len_(len), key_(key) {
...
//-------------------------- setter getter ---------------------------------
  ...
  cache_key_t key() const{ return key_; }

  void setVersion(version_t version) { version_ = version; }
  version_t getVersion() { return version_; }
  version_t getUpdates() { return updates_; }
