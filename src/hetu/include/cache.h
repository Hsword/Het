...
//------------------------- implement main python API ---------------------
  version_t getPullBound() { return pull_bound_; }
  version_t getPushBound() { return push_bound_; }
  size_t getWidth() { return width_; }
  void setPullBound(version_t bound) { pull_bound_ = bound; }
  void setPushBound(version_t bound) { push_bound_ = bound; }
  void bypass() { bypass_cache_ = true; }
  void undoBypass() { bypass_cache_ = false; }
  bool getPerfEnabled() { return perf_enabled_; }
  void setPerfEnabled(bool value) { perf_enabled_ = value; }
  /*
    embeddingLookup is called before each training batch
    * keys may be duplicated, unique operation is required before sending server requests
  */
  wait_t embeddingLookup(py::array_t<cache_key_t> keys, py::array_t<embed_t> dest);
  ...
  wait_t embeddingUpdate(py::array_t<cache_key_t> keys, py::array_t<embed_t> grads);
  ...
  wait_t embeddingPushPullRaw(uint64_t _pullkeys, uint64_t _dest, size_t num_pull_keys,
    uint64_t _pushkeys, uint64_t _grads, size_t num_push_keys);
  void _embeddingPushPull(SArray<cache_key_t> keys, embed_t* dest, SArray<cache_key_t> push_keys, const embed_t* grads);
  std::string __repr__();
  py::list getPerf() { return perf_; };
}; // class CacheBase
