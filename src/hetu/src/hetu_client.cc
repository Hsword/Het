...
size_t syncEmbedding(int node_id, vector<EmbeddingPT> &embed, size_t bound) {
  ...
  auto closure = [embed, width, pulled](const ps::PSFData<ps::kSyncEmbedding>::Response &response, size_t offset) {
    ...
    for (size_t i = 0; i < ret_idx.size(); i++) {
      ...
      std::copy(&value[i * width], &value[(i + 1) * width], embed[idx]->data());
      embed[idx]->addup();
    }
    *pulled = ret_idx.size();
  };
...
void pushEmbedding(int node_id, vector<EmbeddingPT> &embed) {
  // std::cout << "Hetu : Push Embedding: " << embed.size() << std::endl;
  if (embed.empty()) return;
  size_t n = embed.size();
  size_t width = embed[0]->size();
  SArray<cache_key_t> keys(n);
  for (size_t i = 0; i < n; i++) keys[i] = embed[i]->key();
  SArray<embed_t> value(n * width);
  SArray<version_t> updates(n);
  for (size_t i = 0; i < n; i++) {
    std::copy(embed[i]->grad(), embed[i]->grad()+width, &value[i * width]);
    updates[i] = embed[i]->getUpdates();
  }
  ps::PushEmbedding(node_id, keys, value, updates);
}

size_t pushSyncEmbedding(int node_id, vector<EmbeddingPT> &embed, size_t bound, vector<EmbeddingPT> &push_embed) {
  if (embed.empty() && push_embed.empty()) return 0;
  size_t n = embed.size();
  size_t width = embed[0]->size();
  SArray<cache_key_t> keys(n);
  SArray<version_t> vers(n);
  ...
  for (size_t i = 0; i < n; i++) push_keys[i] = push_embed[i]->key();
  SArray<embed_t> value(n * width);
  SArray<version_t> updates(n);
  for (size_t i = 0; i < n; i++) {
    std::copy(push_embed[i]->grad(), push_embed[i]->grad()+width, &value[i * width]);
    updates[i] = push_embed[i]->getUpdates();
  }
  ps::PushSyncEmbedding(node_id, keys, vers, bound, closure, push_keys, value, updates);
