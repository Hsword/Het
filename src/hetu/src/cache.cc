...
vector<EmbeddingPT>
CacheBase::batchedLookup(const cache_key_t * keys, size_t len) {
...
void
CacheBase::batchedInsert(vector<EmbeddingPT> &ptrs) {
  std::lock_guard<std::mutex> lock(mtx);
  if (bypass_cache_)
    return;
  for (auto &ptr: ptrs) {
...
wait_t
CacheBase::embeddingLookup(py::array_t<cache_key_t> _keys, py::array_t<embed_t> _dest) {
  PYTHON_CHECK_ARRAY(_keys);
  PYTHON_CHECK_ARRAY(_dest);
  auto dest = _dest.mutable_data();
  size_t num_keys = _keys.size();
  assert(_dest.size() == num_keys * width_);
  SArray<cache_key_t> keys(_keys.mutable_data(), num_keys);
  return ThreadPool::Get()->Enqueue(&CacheBase::_embeddingLookup, this, keys, dest);
}

wait_t
CacheBase::embeddingLookupRaw(uint64_t _keys, uint64_t dest, size_t num_keys) {
  float *keys = reinterpret_cast<float*>(_keys);
  SArray<cache_key_t> intkeys(num_keys);
  for (size_t i = 0; i < num_keys; i++) intkeys[i] = (cache_key_t)keys[i];
  return ThreadPool::Get()->Enqueue(&CacheBase::_embeddingLookup, this ,intkeys, reinterpret_cast<embed_t*>(dest));
}

void
CacheBase::_embeddingLookup(SArray<cache_key_t> keys, embed_t* dest) {
  ...
  for (size_t _i = 0; _i < keys.size(); _i++) {
    ...
    std::copy(embeds[i]->data(), embeds[i]->data() + embeds[i]->size(), dest + _i * embeds[i]->size());
  }
...
wait_t
CacheBase::embeddingUpdate(py::array_t<cache_key_t> _keys, py::array_t<embed_t> _grads) {
  PYTHON_CHECK_ARRAY(_keys);
  PYTHON_CHECK_ARRAY(_grads);
  auto grads = _grads.data();
  size_t num_keys = _keys.size();
  assert(_grads.size() == num_keys * width_);
  SArray<cache_key_t> keys(_keys.mutable_data(), num_keys);
  return ThreadPool::Get()->Enqueue(&CacheBase::_embeddingUpdate, this, keys, grads);
}

wait_t
CacheBase::embeddingUpdateRaw(uint64_t _keys, uint64_t grads, size_t num_keys) {
  float *keys = reinterpret_cast<float*>(_keys);
  SArray<cache_key_t> intkeys(num_keys);
  for (size_t i = 0; i < num_keys; i++) intkeys[i] = (cache_key_t)keys[i];
  return ThreadPool::Get()->Enqueue(&CacheBase::_embeddingUpdate, this, intkeys, reinterpret_cast<embed_t*>(grads));
}

void
CacheBase::_embeddingUpdate(SArray<cache_key_t> keys, const embed_t* grads) {
  auto start_time = std::chrono::system_clock::now();
  // Unique operation
  auto unique_keys = Unique<cache_key_t>(keys.data(), keys.size());
  auto unique_time = std::chrono::system_clock::now();
  ...
  for (size_t _i = 0; _i < keys.size(); _i++) {
    auto i = unique_keys.map(_i);
    if (!embeds[i]) {
      // !! This is not likely to happen, newly pulled embedding should be in cache
  ...
  for (size_t i = 0; i < unique_keys.size(); i++) {
    if (embeds[i]->getUpdates() > push_bound_ || !embeds[i]->data()) {
      // merge sort
      while (evict_iter!=evict.end() && (*evict_iter)->key() < embeds[i]->key()) should_push.push_back(*evict_iter++);
      should_push.push_back(embeds[i]);
    }
  }
  while (evict_iter != evict.end()) should_push.push_back(*evict_iter++);
  ...
  for (size_t i = 0; i < unique_keys.size(); i++) {
    if (embeds[i]->getUpdates() > push_bound_ && embeds[i]->data()) {
      embeds[i]->setVersion(embeds[i]->getVersion() + embeds[i]->getUpdates());
      embeds[i]->zeroGrad();
    }
  }
  auto end_time = std::chrono::system_clock::now();
...
wait_t CacheBase::embeddingPushPullRaw(uint64_t _pullkeys, uint64_t _dest, size_t num_pull_keys,
  uint64_t _pushkeys, uint64_t _grads, size_t num_push_keys) {
  ...
  for (size_t i = 0; i < num_pull_keys; i++) intpullkeys[i] = (cache_key_t)pullkeys[i];
  for (size_t i = 0; i < num_push_keys; i++) intpushkeys[i] = (cache_key_t)pushkeys[i];
  return ThreadPool::Get()->Enqueue([this, intpullkeys, intpushkeys, _grads, _dest](){
    // _embeddingUpdate(intpushkeys, reinterpret_cast<embed_t*>(_grads));
    // _embeddingLookup(intpullkeys, reinterpret_cast<embed_t*>(_dest));
    _embeddingPushPull(intpullkeys, reinterpret_cast<embed_t*>(_dest), intpushkeys, reinterpret_cast<embed_t*>(_grads));
  });
}

void CacheBase::_embeddingPushPull(SArray<cache_key_t> keys, embed_t* dest, SArray<cache_key_t> push_keys, const embed_t* grads) {
  auto unique_keys = Unique<cache_key_t>(keys.data(), keys.size());
  // Lookup all the keys together
  auto embeds = batchedLookup(unique_keys.data(), unique_keys.size());
  // Scan out missed keys and pull from server
  ...
  auto push_unique_keys = Unique<cache_key_t>(push_keys.data(), push_keys.size());
  auto push_embeds = batchedLookup(push_unique_keys.data(), push_unique_keys.size());
  size_t miss_cnt = 0, evict_cnt = evict_.size();
  vector<EmbeddingPT> should_push;
  vector<EmbeddingPT> evict = std::move(evict_);
  for (size_t _i = 0; _i < push_keys.size(); _i++) {
    ...
    if (!push_embeds[i]) {
      // !! This is not likely to happen, newly pulled embedding should be in cache
      push_embeds[i].reset(new Embedding(push_unique_keys[i], width_, false));
      miss_cnt++;
    }
  ...
  for (size_t i = 0; i < push_unique_keys.size(); i++) {
    if (push_embeds[i]->getUpdates() > push_bound_ || !push_embeds[i]->data()) {
      // merge sort
      while (evict_iter!=evict.end() && (*evict_iter)->key() < push_embeds[i]->key()) should_push.push_back(*evict_iter++);
      should_push.push_back(push_embeds[i]);
    }
  }
  while (evict_iter != evict.end()) should_push.push_back(*evict_iter++);
  size_t pulled = pushSyncEmbedding(node_id_, embeds, pull_bound_, should_push);
  ...
  for (size_t i = 0; i < push_unique_keys.size(); i++) {
    if (push_embeds[i]->getUpdates() > push_bound_ && push_embeds[i]->data()) {
      push_embeds[i]->setVersion(push_embeds[i]->getVersion() + push_embeds[i]->getUpdates());
