...
PYBIND11_MODULE(hetu, m) {
  m.doc() = "hetu C++ plugin"; // optional module docstring
  ...
  .def_property("version", &Embedding::getVersion, &Embedding::setVersion);
  ...
  .def_property("pull_bound", &CacheBase::getPullBound, &CacheBase::setPullBound)
  .def_property("push_bound", &CacheBase::getPushBound, &CacheBase::setPushBound)
  .def_property("perf_enabled", &CacheBase::getPerfEnabled, &CacheBase::setPerfEnabled)
