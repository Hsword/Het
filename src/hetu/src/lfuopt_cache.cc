...
EmbeddingPT LFUOptCache::lookup(cache_key_t k) {
  ...
  if (ptr != store_.end()) return ptr->second;
...
std::list<LFUOptCache::Block>::iterator LFUOptCache::_create(EmbeddingPT embed) {
...
void LFUOptCache::_evict() {
  for (int i = 0; i < kUseCntMax; i++) {
    if (!clist[i].empty()) {
      ...
      clist[i].pop_back();
      if (embed->getUpdates()) evict_.push_back(embed);
      break;
    }
  }
}

std::list<LFUOptCache::Block>::iterator LFUOptCache::_increase(std::list<Block>::iterator iter) {
  size_t use = iter->use;
...
py::array_t<cache_key_t> LFUOptCache::PyAPI_keys() {
  ...
  for (auto &iter : store_) keys.push_back(iter.first);
  for (auto &iter : hash_) keys.push_back(iter.first);
