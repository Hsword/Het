...
extern "C" int cpu_EmbeddingLookup(const DLArrayHandle in_mat, const DLArrayHandle ids, DLArrayHandle out_mat) {
    ...
    size_t idx_size = 1;
    for (int i = 0; i < ids->ndim; ++i) idx_size *= ids->shape[i];

	#pragma omp parallel for
	for (size_t i = 0; i < idx_size; ++i) {
        memcpy(output + i * width, embed + size_t(index[i]) * width, entry_size);
	}
	return 0;
}