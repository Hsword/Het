...
extern "C" int cpu_SGDOptimizerSparseUpdate(DLArrayHandle param, const DLArrayHandle grad_indices, const DLArrayHandle grad_values, float lr)
{
    size_t num = 1;
    for (int i = 0; i < grad_indices->ndim; ++i) num *= grad_indices->shape[i];
    assert (param->ndim == 2);
