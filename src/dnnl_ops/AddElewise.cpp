...
extern "C" int DnnlMatrixElementwiseAdd(const DLArrayHandle matA, const DLArrayHandle matB, DLArrayHandle output)
{
...
//        }
        ...
        for(int i = 0; i < matA -> ndim; i++) shape.push_back(matA -> shape[i]);
        format.resize(matA -> ndim);
        format[(matA -> ndim) - 1] = 1;
        for(int i = format.size() - 2 ; i >= 0; i--) format[i] = format[i+1] * shape[i+1];
        ...
        auto AddElewise_d = binary::desc(algorithm::binary_add, mat_md, mat_md, mat_md);
        auto AddElewise_pd = binary::primitive_desc(AddElewise_d, eng);
        auto AddElewise = binary(AddElewise_pd);
    
        AddElewise.execute(engine_stream, {{DNNL_ARG_SRC_0, srcA_mem}, {DNNL_ARG_SRC_1, srcB_mem}, {DNNL_ARG_DST, dst_mem}});
        engine_stream.wait();
        ...
        for(int i = 0; i < output_size; i++){
            ...
            for (int j = 0; j < matA->ndim; ++j) {
                ...
                if(matA->ndim-j<=matB->ndim && matB->shape[j-(matA->ndim-matB->ndim)]>1){
        ...
        for(int i = 0; i < output_size; i++){
            ...
            for (int j = 0; j < matB->ndim; ++j) {
                ...
                if(matB->ndim-j<=matA->ndim && matA->shape[j-(matB->ndim-matA->ndim)]>1){
