...
def get_token_embeddings(vocab_size, num_units, initializer=init.xavier_normal, zero_pad=True):
    if zero_pad:
        ...
        padding_zero = init.zeros(name='padding_zero', shape=(1, num_units), trainable=False)
...
def multihead_attention(
    ...
    if attention_mask is not None:
        ...
        attention_scores = attention_scores + ad.broadcastto_op(attention_mask, attention_scores)
    if causality:
