...
def multihead_attention(
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        query_layer = tf.layers.dense(queries, hidden_size, activation=query_act) # (N, T_q, d_model)
        key_layer = tf.layers.dense(keys, hidden_size, activation=key_act) # (N, T_k, d_model)
        value_layer = tf.layers.dense(values, hidden_size, activation=value_act) # (N, T_k, d_model)
        ...
        # score
        attention_scores = tf.matmul(query_layer, key_layer, transpose_b=True) # (N, h, T_q, T_k)
        attention_scores /= size_per_head ** 0.5

        # mask
        if attention_mask is not None:
            attention_mask = tf.to_float(attention_mask) # (N, T_k)
            attention_mask = tf.reshape(attention_mask, [batch_size, 1, 1, -1]) # (N, 1, 1, T_k)
            attention_scores = attention_scores + attention_mask * (-2**32+1) # (N, h, T_q, T_k)
        if causality:
            diag_vals = tf.ones_like(attention_scores[0, 0, :, :])  # (T_q, T_k)
            tril = tf.linalg.LinearOperatorLowerTriangular(diag_vals).to_dense()  # (T_q, T_k)
            future_masks = tf.broadcast_to(tril, [batch_size, num_attention_heads, tril.shape[0], tril.shape[1]]) # (N, h, T_q, T_k)
            paddings = tf.ones_like(future_masks) * (-2**32+1)
            attention_scores = tf.where(tf.equal(future_masks, 0), paddings, attention_scores)

        # probs
        attention_probs = tf.nn.softmax(attention_scores) # (N, h, T_q, T_k)
        attention_probs = tf.layers.dropout(attention_probs, rate=attention_probs_dropout_prob, training=training)
        context_layer = tf.matmul(attention_probs, value_layer) # (N, h, T_q, d_model/h)
        context_layer = tf.transpose(context_layer, [0, 2, 1, 3]) # (N, T_q, h, d_model/h)
        outputs = tf.reshape(context_layer, [batch_size, -1, num_attention_heads * size_per_head]) # (N, T_q, d_model)
