...
def test_init_ps(rarr, init_type, init_a, init_b=1.0, sparse=False):
    ...
    init_type_map = {'constant': 0, 'uniform': 1, 'normal': 2, 'truncated_normal': 3}
    ctx = ndarray.cpu(0)
    ...
    comm.InitTensor(ctypes.c_int(0), ctypes.c_int(sparse), arr_len, arr_wid, itype, ctypes.c_double(init_a), ctypes.c_double(init_b))
    ...
    if rank != 0:
        ...
        if init_type == 'constant':
            np.testing.assert_allclose(np.full((nitem, item_len), init_a), arr.asnumpy(), rtol=5e-7)
        else:
            if init_type == 'uniform':
                numpy_samples = np.random.uniform(low=init_a, high=init_b, size=(nitem, item_len)).astype(np.float32)
            elif init_type == 'normal':
                numpy_samples = np.random.normal(loc=init_a, scale=init_b, size=(nitem, item_len)).astype(np.float32)
            else:
                numpy_samples = truncnorm.rvs(-2.0, 2.0, loc=init_a, scale=init_b, size=(nitem, item_len)).astype(np.float32)
            fig, ax = plt.subplots(1, 1)
            ax.hist(numpy_samples.flatten(), histtype='stepfilled', alpha=0.2, bins=50, label='numpy')
            ax.hist(local_arr.flatten(), histtype='step', alpha=0.2, bins=50, label='ps')
            ax.legend(loc='best', frameon=False)
            # ax2.legend(loc='best', frameon=False)
            file_name = '%s_%.1f_%.1f_%d.png' % (init_type, init_a, init_b, int(sparse))
...
def test_api(rarr, rpush, rpull, sparse=False):
    ctx = ndarray.cpu(0)
    rank = int(os.environ["WORKER_ID"])
    nrank = int(os.environ["DMLC_NUM_WORKER"])
    local_arr = np.frombuffer(rarr, dtype=np.float32).reshape(nitem, item_len).copy()
    ...
    if sparse:
        ...
        arr_len = ctypes.c_int(nitem * item_len)
        arr_wid = ctypes.c_int(1)
    comm.InitTensor(ctypes.c_int(0), ctypes.c_int(sparse), arr_len, arr_wid, ctypes.c_int(0), ctypes.c_double(0.0), ctypes.c_double(1.0))
    if sparse:
        local_arr[:] = 0
        for j in local_push:
            local_arr[int(j)] += 1
        if rank == 0:
            ...
            push_val = ndarray.array(np.ones((indx1, indx2, item_len)).astype(np.float32), ctx=ctx)
        ...
        if rank == 0:
            push_ind = ndarray.array(local_push.reshape(indx1, indx2), ctx=ctx)
            push_val = ndarray.array(np.ones((indx1, indx2, item_len)).astype(np.float32), ctx=ctx)
            comm.SDPushPull(0, push_ind.handle, push_val.handle, arr.handle, None)
            comm.Wait(0)
        comm.BarrierWorker()
        if rank != 0:
            comm.Pull(0, arr.handle)
        ...
        if rank != 0:
            ...
            comm.Wait(0)
        np.testing.assert_allclose(local_arr[local_pull.astype(int)].reshape(indx1, indx2, item_len), pull_val.asnumpy(), rtol=5e-7)
        ...
        comm.BarrierWorker()
        if rank == 0:
            temp_push_val = ndarray.array(np.ones((nitem, item_len)).astype(np.float32), ctx=ctx)
...
if __name__ =='__main__':
    ...
    parser.add_argument("--config", default='./settings/local_s2_w2.yml')
    ...
    push_arr = rarr('f', np.random.randint(0, nitem, (indx1 * indx2)).astype(np.float32))
    pull_arr = rarr('f', np.random.randint(0, nitem, (indx1 * indx2)).astype(np.float32))
    for key, value in settings.items():
        if key != 'shared':
            if key[0] != 'w':
                proc = multiprocessing.Process(target=start_process, args=[value, args])
            else:
                proc = multiprocessing.Process(target=start_process, args=[value, args, arr, push_arr, pull_arr])
