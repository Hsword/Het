...
def wdl_adult(X_deep, X_wide, y_, cluster=None, task_id=None):
    ...
    with device:
        ...
        W = tf.Variable(rand.normal(scale=0.1, size=[dim_wide+20, 2]), dtype = tf.float32)
        W1 = tf.Variable(rand.normal(scale=0.1, size=[dim_deep, 50]), dtype = tf.float32)
        b1 = tf.Variable(rand.normal(scale=0.1, size=[50]), dtype = tf.float32)
        W2 = tf.Variable(rand.normal(scale=0.1, size=[50, 20]), dtype = tf.float32)
        b2 = tf.Variable(rand.normal(scale=0.1, size=[20]), dtype = tf.float32)

        Embedding = []

        for i in range(8):
            Embedding.append(tf.Variable(rand.normal(scale=0.1, size=[20, 8]), dtype = tf.float32))
        ...
        dropout1 = relu1 #tf.nn.dropout(relu1, 0.5)
        mat2 = tf.matmul(dropout1, W2)
        add2 = tf.add(mat2, b2)
        relu2= tf.nn.relu(add2)
        dropout2 = relu2 #tf.nn.dropout(relu2, 0.5)
        dmodel=dropout2
