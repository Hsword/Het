...
def main():
    ...
    parser.add_argument("--config", type=str, required=True, help="configuration for ps")
    ...
    executor = ad.Executor([loss, prediction, y_, train_op], ctx=ndarray.gpu(0),\
        comm_mode='PS', dataloader_name='train', stream_mode='AllStreams', use_sparse_pull=True, cstable_policy=args.cache, bsp=args.bsp, cache_bound=args.bound)
    if args.val:
        val_executor = ad.Executor([loss, prediction, y_], ctx=ndarray.gpu(0),\
            dataloader_name='validate', stream_mode='AllStreams', comm_mode='PS', use_sparse_pull=True, cstable_policy=args.cache, inference=True)
            
    if args.all:
        raw_log_file = './logs/worker_%s' % (args.model)
        if args.bsp:
            raw_log_file += '_bsp'
        else:
            raw_log_file += '_asp'
        ...
        raw_log_file += '_%d.log' % (int(executor.config.worker_id.lstrip('./W')))
