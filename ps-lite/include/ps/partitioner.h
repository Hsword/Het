...
public:
    ...
    virtual ~Partitioner() {}
    virtual void partitionDense(size_t length, std::vector<Key> &keys, std::vector<size_t> &parts) {}
    virtual void partitionSparse(size_t length, size_t width, std::vector<Key> &keys, std::vector<size_t> &parts) {}
    virtual int queryServer(Key key) {return 0;}
};
...
public:
    AveragePartitioner(size_t part_num=0): Partitioner() {
        _globalId = 0;
        _serverIndex = 0;
        if (part_num == 0 || part_num > server_num) part_num = server_num;
        partition_num = part_num;
    }

    void partitionDense(size_t length, std::vector<Key> &keys, std::vector<size_t> &parts) {
    ...
    void partitionSparse(size_t length, size_t width, std::vector<Key> &keys, std::vector<size_t> &parts) {
...
public:
    ...
    void partitionImpl(size_t length, size_t cur_block, std::vector<Key> &keys, std::vector<size_t> &parts) {
