...
template<> struct PSFData<SparsePull> {
  using Request = tuple<
    Key, // key
  ...
  static void _callback(const Response &response, SArray<float> tgt, 
        std::vector<std::pair<size_t, std::vector<size_t> > > mapping, size_t offset, size_t width) {
    auto val = get<0>(response);
    CHECK_EQ(val.size(), mapping.size() * width) << val.size() << " " << mapping.size() << " " << width;
...
template<> struct PSFData<SparsePush> {
  using Request = tuple<
    Key, // key
    SArray<size_t>, // offset
    SArray<float> // data
...
template<> struct PSFData<SDPushPull> {
  using Request = tuple<
    Key, // key
    SArray<size_t>, // offset
    SArray<float>, // data
...
template<> struct PSFData<SSPushPull> {
  using Request = tuple<
    Key, // key
