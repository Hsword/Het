...
public:
  ...
  void parameter_init(int node_name, ParamType ptype, size_t len, size_t width, InitType init_type, double init_a=0.0, double init_b=1.0);
  ...
  */
  query_t push_data(const long *indices, int index_size, const DLArray *value, const long *lengths);
  // this is almost the same as push_data
  query_t pull_data(const long *indices, int index_size, DLArray *value, const long *lengths);
  /*
  ...
  */
  ...
  void dd_pushpull(int node_name, const DLArray* in_arr, DLArray *out_arr, DLEvent *evt);
  void sparse_pull(int node_name, const DLArray *index, DLArray *value, size_t index_size);
  void sparse_push(int node_name, const DLArray *index, const DLArray *value, size_t index_size, DLEvent *evt);
  void sd_pushpull(int node_name, const DLArray* index, const DLArray* in_arr, size_t index_size, DLArray *out_arr, DLEvent *evt);
  void ss_pushpull(int node_name, const DLArray* inind, const DLArray* in_arr, const DLArray* outind, DLArray* out_arr, size_t index_size, DLEvent *evt);
