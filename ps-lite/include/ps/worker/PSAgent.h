...
public:
     ...
     */
    void registerTensor(const int name, const ParamType ptype, const size_t length, const size_t width = 1) {
    ...
    void vecPushSparse(const int name, float* dup_index, float* vals, const size_t dup_index_size, int priority = 0) {
        TensorMeta& meta = _id2meta[name];
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            ...
            if (cur_index > st_index) {
                ...
                PSFData<SparsePush>::Request request(
                    keys[i], SArray<size_t>(cp_offset + st_index, cur_index - st_index), SArray<float>(cp_val + st_offset, cur_offset - st_offset));
    ...
    void vecPullSparse(const int name, float* dup_index, float* vals, const size_t dup_index_size, int priority = 0) {
        TensorMeta& meta = _id2meta[name];
        const std::vector<Key> &keys = meta.keys;
        const std::vector<size_t> &lens = meta.part;
        size_t width = meta.width;
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            ...
            if (cur_index > st_index) {
                ...
                PSFData<SparsePull>::Request request(
                    keys[i], SArray<size_t>(cp_offset + st_index, cur_index - st_index));
                auto cb = getCallBack<SparsePull>(SArray<float>(vals, dup_index_size * width),
                    std::move(std::vector<std::pair<size_t, std::vector<size_t> > >(st_iter, iter)), cur_len, width);
    ...
    void vecSDPushPull(const int name, float* dup_index, float* vals, const size_t dup_index_size, float *out_vals, int priority = 0) {
        TensorMeta& meta = _id2meta[name];
        const std::vector<Key> &keys = meta.keys;
        const std::vector<size_t> &lens = meta.part;
        size_t width = meta.width;
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            ...
            PSFData<SDPushPull>::Request request(
                keys[i], SArray<size_t>(cp_offset + st_index, cur_index - st_index), SArray<float>(cp_val + st_offset, cur_offset - st_offset), local_length);
            auto cb = getCallBack<SDPushPull>(SArray<float>(out_vals + pull_offset, local_length));
    ...
    void vecSSPushPull(const int name, float* in_index, float* in_vals, float* out_index, float* out_vals, const size_t dup_index_size, int priority = 0) {
        TensorMeta& meta = _id2meta[name];
        const std::vector<Key> &keys = meta.keys;
        const std::vector<size_t> &lens = meta.part;
        size_t width = meta.width;
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            ...
            while (in_iter != in_idx2map.end() && in_iter->first < cur_len + lens[i]) {
            ...
            while (out_iter != out_idx2map.end() && out_iter->first < cur_len + lens[i]) {
            ...
            if (in_cur_index > in_st_index || out_cur_index > out_st_index) {
                ...
                PSFData<SSPushPull>::Request request(
                    keys[i],
                    SArray<size_t>(in_cp_offset + in_st_index, in_cur_index - in_st_index),
                    SArray<float>(in_cp_val + st_offset, in_cur_offset - st_offset),
                    SArray<size_t>(out_cp_offset + out_st_index, out_cur_index - out_st_index)
                );
                auto cb = getCallBack<SparsePull>(SArray<float>(out_vals, dup_index_size * width),
                    std::move(std::vector<std::pair<size_t, std::vector<size_t> > >(st_iter, out_iter)), cur_len, width);
    ...
    void vecDensePush(const int name, float* vals, int priority = 0) {
        ...
        size_t cur_len = 0;
        for(size_t i = 0; i < meta.keys.size(); i++) {
            PSFData<DensePush>::Request request(meta.keys[i], meta.part[i], SArray<float>(vals + cur_len, meta.part[i]));
    ...
    void vecDensePull(const int name, float *vals, int priority = 0) {
        ...
        for(size_t i = 0; i < meta.keys.size(); i++) {
            ...
            auto cb = getCallBack<DensePull>(SArray<float>(vals + cur_offset, cur_length));
            meta.ts.push_back(_kvworker.Request<DensePull>(request, cb));
            cur_offset += cur_length;
        }
    }

    void vecDDPushPull(const int name, float *in_vals, float *out_vals, int priority = 0) {
        TensorMeta &meta = _id2meta[name];
        size_t cur_len = 0;
        /* send pull request to each partition */
        for (size_t i = 0; i < meta.keys.size(); i++) {
            PSFData<DDPushPull>::Request request(meta.keys[i], meta.part[i], SArray<float>(in_vals + cur_len, meta.part[i]));
            auto cb = getCallBack<DDPushPull>(SArray<float>(out_vals + cur_len, meta.part[i]));
            meta.ts.push_back(_kvworker.Request<DDPushPull>(request, cb));
            cur_len += meta.part[i];
        }
    }

    void ParameterInit(const int name, InitType init_type, double init_a, double init_b) {
        TensorMeta &meta = _id2meta[name];
        /* send pull request to each partition */
        auto cb = getCallBack<ParamInit>();
        for(size_t i = 0; i < meta.keys.size(); i ++) {
            PSFData<ParamInit>::Request request(meta.keys[i], meta.ptype, meta.part[i], meta.width, init_type, init_a, init_b);
    ...
    void ParameterSave(const int name, char* address) {
        ...
        for(size_t i = 0; i < meta.keys.size(); i ++) {
            std::string local_address = std::string(address) + "/" + std::to_string(name) + "_" + std::to_string(i) + ".dat";
            SArray<char> temp_array;
            temp_array.CopyFrom(local_address.c_str(), local_address.size());
            PSFData<ParamSave>::Request request(meta.keys[i], temp_array, false);
    ...
    */
    ...
    void syncEmbedding(int name, const SArray<uint64_t> &rows, const SArray<version_t> &ver, version_t bound,
        ...
        for (size_t i = 0; i < meta.keys.size(); i++) {
            // find the idx range
            start = end;
            end = std::lower_bound(rows.begin() + start, rows.end(), cur_len +  meta.part[i]) - rows.begin();
            if (start == end) continue;
            // remove row offset inplace so that index fits with server
            SArray<uint64_t> new_rows = rows.segment(start, end);
            for (size_t i = 0; i < new_rows.size(); i++) new_rows[i] -= cur_len;
            PSFData<kSyncEmbedding>::Request request(meta.keys[i], new_rows, ver.segment(start, end), bound);
    ...
    void pushEmbedding(int name, const SArray<uint64_t> &rows, const SArray<float> &data, const SArray<version_t> &updates) {
        TensorMeta &meta = _id2meta[name];
        size_t start = 0, end = 0, cur_len = 0;
        ...
        for (size_t i = 0; i < meta.keys.size(); i++) {
            ...
            SArray<uint64_t> new_rows = rows.segment(start, end);
            for (size_t i = 0; i < new_rows.size(); i++) new_rows[i] -= cur_len;
            PSFData<kPushEmbedding>::Request request(meta.keys[i], new_rows,
                data.segment(start * meta.width, end * meta.width), updates.segment(start, end));
            meta.ts.push_back(_kvworker.Request<kPushEmbedding>(request, cb));
            cur_len += meta.part[i];
        }
    }

    void pushSyncEmbedding(int name, const SArray<uint64_t> &rows, const SArray<version_t> &ver, version_t bound,
    PSFData<kSyncEmbedding>::Closure closure,
    const SArray<uint64_t> &push_rows, const SArray<float> &data, const SArray<version_t> &updates) {
        TensorMeta &meta = _id2meta[name];
        ...
        for (size_t i = 0; i < meta.keys.size(); i++) {
            ...
            push_end = std::lower_bound(push_rows.begin() + push_start, push_rows.end(), cur_len +  meta.part[i]) - push_rows.begin();
            if (start == end && push_start == push_end) continue;
            // remove row offset inplace so that index fits with server
            SArray<uint64_t> new_rows = rows.segment(start, end), new_push_rows = push_rows.segment(push_start, push_end);
            for (size_t i = 0; i < new_rows.size(); i++) new_rows[i] -= cur_len;
            for (size_t i = 0; i < new_push_rows.size(); i++) new_push_rows[i] -= cur_len;
            PSFData<kPushSyncEmbedding>::Request request(
                meta.keys[i], new_rows, ver.segment(start, end), bound,
                new_push_rows, data.segment(push_start * meta.width, push_end * meta.width), updates.segment(push_start, push_end));
            auto cb = std::bind(closure, std::placeholders::_1, start);
            meta.ts.push_back(_kvworker.Request<kPushSyncEmbedding>(request, cb));
            cur_len += meta.part[i];
        }
    }
};
