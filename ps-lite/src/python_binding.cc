...
void DDPushPull(int node_name, const DLArray* in_arr, DLArray *out_arr, DLEvent *evt) {
    worker.dd_pushpull(node_name, in_arr, out_arr, evt);
}

void SparsePush(int node_name, const DLArray *index, const DLArray *value, DLEvent *evt) {
    size_t index_size = 1;
    for(int i = 0; i < index->ndim; i++) index_size *= index->shape[i];
...
void SDPushPull(int node_name, const DLArray* index, const DLArray* in_arr, DLArray *out_arr, DLEvent *evt) {
    size_t index_size = 1;
    for (int i = 0; i < index->ndim; i++) index_size *= index->shape[i];
    worker.sd_pushpull(node_name, index, in_arr, index_size, out_arr, evt);
}

void SSPushPull(int node_name, const DLArray* inindices, const DLArray* in_arr, const DLArray* outindices, DLArray* out_arr, DLEvent *evt) {
    size_t index_size = 1;
    ...
    worker.ss_pushpull(node_name, inindices, in_arr, outindices, out_arr, index_size, evt);
}
...
*/
Worker::query_t PushData(const long *index, int index_size, const DLArray *value, const long *length) {
    return worker.push_data(index, index_size, value, length);
}

Worker::query_t PullData(const long *index, int index_size, DLArray *value, const long *length) {
...
void InitTensor(int node_name, int ptype, int len, int width, int init_type, double init_a, double init_b) {
    worker.parameter_init(node_name, static_cast<ParamType>(ptype), static_cast<size_t>(len), static_cast<size_t>(width),
        static_cast<InitType>(init_type), init_a, init_b);
}
