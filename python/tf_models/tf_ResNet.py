...
def tf_conv2d(x, in_channel, out_channel, stride=1):
    weight = tf.Variable(np.random.normal(scale=0.1, size=(out_channel, in_channel, 3, 3)).transpose([2, 3, 1, 0]).astype(np.float32))
    x = tf.nn.conv2d(x, weight, strides=[1, stride, stride, 1], padding='SAME')
    return x


def tf_batch_norm_with_relu(x, hidden):
    scale = tf.Variable(np.random.normal(scale=0.1, size=(hidden,)).astype(np.float32))
    bias = tf.Variable(np.random.normal(scale=0.1, size=(hidden,)).astype(np.float32))
    axis = list(range(len(x.shape) - 1))
    a_mean, a_var = tf.nn.moments(x, axis)
    x = tf.nn.batch_normalization(x, mean=a_mean, variance=a_var, scale=scale, offset=bias, variance_epsilon=1e-2)
    x = tf.nn.relu(x)
    return x
...
def tf_resnet_block(x, in_channel, num_blocks, is_first=False):
    if is_first:
        ...
        x = tf_batch_norm_with_relu(x, out_channel)
        x = tf_conv2d(x, out_channel, out_channel, stride=1)
        identity = tf.nn.avg_pool(identity, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
        identity = tf.pad(identity, [[0, 0], [0, 0], [0, 0], [in_channel // 2, in_channel // 2]])
        x = x + identity
...
def tf_fc(x, shape):
    weight = tf.Variable(np.random.normal(scale=0.1, size=shape).astype(np.float32))
    bias = tf.Variable(np.random.normal(scale=0.1, size=shape[-1:]).astype(np.float32))
    x = tf.matmul(x, weight) + bias
    return x


def tf_resnet(x, y_, num_layers):
    '''
    ...
    y = tf_fc(x, (128 * base_size, 10))
    loss = tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=y_)
    loss = tf.reduce_mean(loss)
    return loss, y


def tf_resnet18(x, y_):
    return tf_resnet(x, y_, 18)


def tf_resnet34(x, y_):
    return tf_resnet(x, y_, 34)
