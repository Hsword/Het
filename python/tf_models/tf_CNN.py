...
def tf_conv_relu_avg(x, shape):
    weight = tf.Variable(np.random.normal(scale=0.1, size=shape).transpose([2, 3, 1, 0]).astype(np.float32))
    x = tf.nn.conv2d(x, weight, padding='SAME', strides=[1, 1, 1, 1])
    x = tf.nn.relu(x)
    x = tf.nn.avg_pool(x, ksize=[1, 2, 2, 1], padding='VALID', strides=[1, 2, 2, 1])
    return x
