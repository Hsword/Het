...
@athena_op(["MatMulOp"],onnx_op=["MatMul"])
class MatMul:
    @classmethod
    def version_1(cls,ctx,node,**kwargs):
        ...
        if trans_a != 0:
            ctx.insert_new_node_on_input(node,'Transpose',node._inputs[0],perm=[1,0])
        if trans_b != 0:
            ctx.insert_new_node_on_input(node,'Transpose',node._inputs[1],perm=[1,0])
