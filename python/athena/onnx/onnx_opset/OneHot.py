...
@athena_op(["OneHotOp"],onnx_op=["OneHot"])
class OneHot:
    @classmethod
    ...
    def version_9(cls, ctx, node, **kwargs):
        ...
        depth_node=ctx.make_const(util.make_name(node.name),np.array([depth],dtype=np.int64))
        values_node=ctx.make_const(util.make_name(node.name),np.array([0,1],dtype=np.int64))
