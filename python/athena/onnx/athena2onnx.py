...
import athena.onnx.onnx_opset
from athena import gpu_ops as ad
from athena.onnx import util,constants,handler
from athena.onnx.graph import Graph
...
def export(
    ...
    onnx_graph=ProcessAthenaGraph(
        executor,inputs,outputs,
    )
    if job_name is None:
        job_name='AthenatoOnnx'
    ...
    with open(onnx_save_dir,'wb') as f:
        try:
            ...
            raise ValueError("Error occured when running model_proto.SerializeToString.")

    # node_list,input_list,output_list=AthenaToOnnxNaive(executor,inputs,outputs)
...
def ProcessAthenaGraph(
    ...
    (onnx_nodes,dtypes,shapes,output_names)=AthenaToOnnxPrevious(executor,inputs,outputs)

    g=Graph(onnx_nodes,shapes,dtypes,opset,output_names=output_names)

    ops_mapping=handler.athena_op.create_mapping(g._opset)
    AthenaOnnxMapping(g,ops_mapping)
    g.topology_sort(g._nodes)
    return g



def AthenaOnnxMapping(g,ops_mapping):
    ...
    for node in ops:
        ...
        assert map_info is not None,"op [%s:%s] is not supported"%(node.name,op)
...
def AthenaToOnnxPrevious(executor,inputs,outputs):
    ...
    for node in topo_nodes:
        ...
        for k,v in kvs.items():
            ...
            #
            if isinstance(v, athena.ndarray.NDArray):
                v = v.asnumpy()
            #for placeholder
        ...
        try:
            ...
            if node in outputs:
                try:
                    assert len(attrs['outputs'])==1,"Failed: output node %s must have one output" %(node.name)

                    defined_out_name='defined_out'+attrs['outputs'][0][attrs['outputs'][0].rfind(':'):]
                    onnx_node = helper.make_node('Identity',inputs=attrs['outputs'],
                                                 outputs=[defined_out_name],
                                                 name=defined_out_name)
                    # fixme:athena tensor have only one dtype as np.float now.
                    ...
                    print("convert failed for %s to defined_out, ex=%s" % (node.name, ex))
                    raise
