...
from athena.onnx import util
from athena.onnx.util import FindOpset
...
class Node(object):
    ...
    def is_const(self):
        return self.op_type in ['Const'] or \
               (self.op_type in ['PlaceholderOp'] and self._attrs.get('value') is not None)
...
class Graph(object):
    ...
    def update_node_shape_dtype(self,node):
        ...
        shapes,dtypes=util.InferOnnxShapeDtype(node,self._opset,input_shapes,input_dtypes,initializers)
    ...
    def replace_input(self,node,old_input,new_input,input_index=None):
        if input_index is None:
            ...
            raise RuntimeError("Failed:Unable to replace input %r into %r for node %r." % (old_input, new_input, node.name))
    ...
    def make_model(self,graph_doc,onnx_filename,graph_name='athena.python.onnx'):
    ...
    def make_graph(self,doc,onnx_filename,graph_name='athena.python.onnx'):
