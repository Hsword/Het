from athena.gpu_ops.executor import path_to_lib
...
class CacheSparseTable:
    def __init__(self, limit, length, width, node_id, policy="LRU", bound=100):
        ...
        policy = policy.lower()
        if policy == "lru":
            self.cache = hetu.LRUCache(limit, length, width, node_id)
        elif policy == "lfu":
            self.cache = hetu.LFUCache(limit, length, width, node_id)
        elif policy == "lfuopt":
            self.cache = hetu.LFUOptCache(limit, length, width, node_id)
        else:
