...
class MatrixDotOp(Op):
    ...
    def gradient(self, output_grad):
        return [matrix_dot_op(output_grad, self.inputs[1], axes=1, ctx=output_grad.ctx), \
            reduce_sum_op(mul_op(self.inputs[0], output_grad, ctx=output_grad.ctx), axes = 1, keepdims=True, ctx = output_grad.ctx)]
