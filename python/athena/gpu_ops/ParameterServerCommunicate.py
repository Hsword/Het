...
class ParameterServerCommunicateOp(Op):
    ...
    def __init__(self, nodeA, parameter, learning_rate):
    ...
    def forward_hook(self, config):
        ...
        if config.cstable_policy is not None and self.parameter.is_embed:
            ...
            from athena.cstable import CacheSparseTable
            ...
            # ALREADY has barrier!
            self.parameter.initializer.init_on_ps(self.comm, self.ps_id, 2)
            ...
            if config.prefetch:
                self.dl_name = config.dataloader_name
        ...
        # this function synchronously initialize meta information and do the initialization,
        # ALREADY has barrier!
        self.parameter.initializer.init_on_ps(self.comm, self.ps_id, int(self.parameter.is_embed))
        if self_sparse:
            if config.prefetch:
                self.dl_name = config.dataloader_name
                self.dl_node = self.inputs[0].inputs[1]
...
def parameterServerCommunicate_op(node, parameter, learning_rate):
    ...
    """
    return ParameterServerCommunicateOp(node, parameter, learning_rate)
