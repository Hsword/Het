...
class Batch_NormalizationOp(Op):
        ...
        if self.on_cpu:
            if DNNL_LIB['DnnlBatchNorm']:
                if self.save_mean is None:
                    ...
                    self.save_mean = np.zeros([C], dtype=np.float32)
                    self.save_var = np.zeros([C], dtype=np.float32)
                cpu_batch_norm(input_vals[0], input_vals[1], input_vals[2], output_val, \
                    self.save_mean, self.save_var, self.momentum, self.eps)
            else:
                output_val[:], self.save_mean, self.save_var = batchnorm_forward(input_vals[0].asnumpy(), input_vals[1].asnumpy(),
                                                                                 input_vals[2].asnumpy(), self.save_mean,
            ...
            CuDNN_Batch_Normalization(
                input_vals[0], input_vals[1], input_vals[2], output_val, self.save_mean, self.save_var, self.momentum,
    ...
    def gradient(self, output_grad):
        ...
        data_gradient = batch_normalization_gradient_of_data_op(
            bn_gradient_node, self.inputs[0], ctx=output_grad.ctx)
        scale_gradient = batch_normalization_gradient_of_scale_op(
            bn_gradient_node, self.inputs[1], ctx=output_grad.ctx)
        bias_gradient = batch_normalization_gradient_of_bias_op(
            bn_gradient_node, self.inputs[2], ctx=output_grad.ctx)
    ...
    def compute(self, input_vals, output_val, stream_handle=None):
        ...
        if self.on_cpu:
            if DNNL_LIB['DnnlBatchNorm_Gradient']:
                ...
                cpu_batch_norm_gradient(input_vals[0], input_vals[1], input_vals[2], bn_bias, self.tmp_gradient_in_arr, self.tmp_gradient_bn_scale,
                                        self.tmp_gradient_bn_bias, self.forward_node.save_mean, self.forward_node.save_var, self.eps)
            else:
                if self.tmp_gradient_bn_bias is None:
                ...
                self.tmp_gradient_in_arr, self.tmp_gradient_bn_scale, self.tmp_gradient_bn_bias = batchnorm_backward(
                    input_vals[0].asnumpy(), input_vals[1].asnumpy(), input_vals[2].asnumpy(), self.tmp_gradient_bn_scale, self.tmp_gradient_bn_bias,
...
def batchnorm_forward(x, bn_scale, bn_bias, save_mean, save_var, momentum=0.99, eps=0.01):
    ...
    return out, sample_mean, sample_var
