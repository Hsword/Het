...
def conv2d_gradient_of_data(input_f, gradient_y, gradient_x, padding=0, stride=1):
    ...
    _LIB.DnnlConv2d_Gradient_of_Data(input_f.handle, gradient_y.handle, gradient_x.handle, ctypes.c_int(padding), ctypes.c_int(stride))

def conv2d_gradient_of_filter(input_x, gradient_y, gradient_f, padding=0, stride=1):
    assert isinstance(gradient_y, _nd.NDArray)
    assert isinstance(input_x, _nd.NDArray)
    assert isinstance(gradient_f, _nd.NDArray)
    _LIB.DnnlConv2d_Gradient_of_Filter(input_x.handle, gradient_y.handle, gradient_f.handle, ctypes.c_int(padding), ctypes.c_int(stride))

def avg_pool(input, H, W, output, padding=0, stride=1):
    assert isinstance(input, _nd.NDArray)
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlAvgPool(input.handle, ctypes.c_int(H), ctypes.c_int(W), output.handle, ctypes.c_int(padding), ctypes.c_int(stride))

def avg_pool_gradient(gradient_Y, H, W, gradient_X, padding=0, stride=1):
    assert isinstance(gradient_Y, _nd.NDArray)
    assert isinstance(gradient_X, _nd.NDArray)
    _LIB.DnnlAvgPool_Gradient(gradient_Y.handle, ctypes.c_int(H), ctypes.c_int(W), gradient_X.handle, ctypes.c_int(padding), ctypes.c_int(stride))

def max_pool(input, H, W, output, padding=0, stride=1):
    assert isinstance(input, _nd.NDArray)
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlMaxPool(input.handle, ctypes.c_int(H), ctypes.c_int(W), output.handle, ctypes.c_int(padding), ctypes.c_int(stride))

def max_pool_gradient(input, input_grad, H, W, output, padding=0, stride=1):
    assert isinstance(input, _nd.NDArray)
    assert isinstance(output, _nd.NDArray)
    _LIB.DnnlMaxPool_Gradient(input.handle, input_grad.handle, ctypes.c_int(H), ctypes.c_int(W), output.handle, ctypes.c_int(padding), ctypes.c_int(stride))
