...
class Dataloader(object):
    def __init__(self, raw_data, batch_size, name='', func=None, drop_last=True):
    ...
    def init_states(self, local_rank=-1, all_ranks=-1):
        if local_rank >= 0 and all_ranks > 0:
            avg = len(self.raw_data) // all_ranks
            start = avg * local_rank
            end = start + avg if local_rank != all_ranks - 1 else len(self.raw_data)
            self.raw_data = self.raw_data[start:end]
...
class DataloaderOp(Op):
    ...
    def forward_hook(self, config):
        assert config.dataloader_name in self.dataloaders.keys(), \
            'Executor uses %s dataloader, not included in (%s).' % (config.dataloader_name, ', '.join(self.dataloaders.keys()))

    def backward_hook(self, config):
        if config.comm_mode is not None:
            local_rank = config.worker_id.strip('./W')
            all_ranks = config.worker_num
            for d in self.dataloaders.values():
                d.init_states(int(local_rank), int(all_ranks))
        else:
            for d in self.dataloaders.values():
                d.init_states()
